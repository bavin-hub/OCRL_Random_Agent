...
class ManagerBasedRlEnv:
  ...
  def __init__(
    ...
    **kwargs,
    ...
    self.db_dir = db_path
    print("db_path : ", self.db_dir)
    print("\n\n\n\n\n\n")
    self.buffer_size_d = 1000
    self.transitions_per_trajectory = 50
  ...
  def sample_st_np(self, env_id: int = 0) -> np.ndarray:
    ...
    return np.concatenate(
      [base_lin_vel, base_ang_vel, gravity_proj, joint_pos, joint_vel, joint_torque],
      axis=-1,
    )
  

  def extract_observation_vectors(self, st, policy_at):

    def scale_policy_outputs(policy_at):
      return ((policy_at * self.my_scale) + self.offset).cpu().numpy().squeeze()

    ct = [0] * 30
    target_actions = scale_policy_outputs(policy_at)
  ...
  def step(self, action: torch.Tensor) -> types.VecEnvStepReturn:
    ...
    *before* integration, so after stepping, derived quantities (``xpos``,
    ...
    """
    # Pre-step sim state for env 0 (before this action is applied); same timing as viewer get_state.
    st = self.sample_st_np(0)


    # change starts #
    self.incremental_step += 1
    self.extract_observation_vectors(st=st, policy_at=action)
